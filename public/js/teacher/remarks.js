...
function renderTable() {
  if (!studentList.length) {
    evaluationTableBody.innerHTML = `<tr><td colspan="13" class="px-6 py-8 text-center text-slate-500">No students found in this class.</td></tr>`;
    return;
  }
  ...
  evaluationTableBody.innerHTML = studentList.map((s) => {
    ...
    return `
      <tr class="hover:bg-slate-50/50 border-b border-slate-100" data-student-id="${s.id}">
        ...
        <td class="px-2 py-3">
          <input type="text" data-remark="teacher" value="${ev.teacher_remark || ""}" class="w-full px-3 py-1.5 border border-slate-200 rounded-lg text-xs outline-none focus:border-indigo-500" placeholder="e.g. Attentive learner" />
        </td>
        <td class="px-2 py-3">
          <input type="text" data-remark="principal" value="${ev.principal_remark || ""}" class="w-full px-3 py-1.5 border border-slate-200 rounded-lg text-xs outline-none focus:border-indigo-500" placeholder="e.g. Promising result" />
        </td>
      </tr>
    `;
  }).join("");
...
async function loadClassOptions() {
  ...
  (assignments || []).forEach((asg) => {
    if (asg.classes) uniqueClasses.set(asg.class_id, asg.classes.name);
  });
  ...
  classSelect.innerHTML = Array.from(uniqueClasses.entries())
...
async function loadData() {
  ...
  evaluationTableBody.innerHTML = `<tr><td colspan="13" class="px-6 py-8 text-center text-slate-500">Loading students and evaluations...</td></tr>`;
  ...
  try {
    ...
    evaluationTableBody.innerHTML = `<tr><td colspan="13" class="px-6 py-8 text-center text-red-500">Failed to load data: ${error.message}</td></tr>`;
  }
}
...
async function saveEvaluations() {
  ...
  rows.forEach((tr) => {
    ...
    const record = {
      ...
      submitted_by: currentTeacher.id,
  ...
  try {
    ...
    saveStatus.textContent = "All changes successfully saved to database!";
...
async function init() {
  try {
    ...
    console.error("Remarks init error:", error);
    authLoader.innerHTML = `
      ...
      <p class="text-sm font-medium text-slate-800 mb-4 text-center">Cannot load evaluations. Please check your connection.</p>
